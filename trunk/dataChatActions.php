...
if(!empty($_POST["sil"]) && $_POST["silIzin"]=="evet") {
   ...
   for ($i = 0; $i < count($_POST["sil"]); $i++)
	...
	if ($sonuc) 
	 echo "<font id='uyari'>Se&ccedil;ilen kayıt(lar) silinmiştir</font>";
	 else
...
?>
  ...
  ?>
	   ...
	   if ($row_eoUsers['ip']==1) echo "Sınıf1"; else
	   if ($row_eoUsers['ip']==2) echo "Sınıf2"; else
	   if ($row_eoUsers['ip']==3) echo "Sınıf3"; else
	   if ($row_eoUsers['ip']==4) echo "Sınıf4"; else
	   if ($row_eoUsers['ip']==5) echo "Sınıf5"; else
	   if ($row_eoUsers['ip']==6) echo "Sınıf6"; else
	   if ($row_eoUsers['ip']==7) echo "Sınıf7"; else
	   if ($row_eoUsers['ip']==9) echo "Sınıf9"; else
	   if ($row_eoUsers['ip']==8) echo "Sınıf8"; 
	  ?></td>
...
if ($totalRows_eoUsers==0) echo "<font id='hata'> Arama sonucuna uyan bilgi bulunamadı veya hi&ccedil; kayıt yok!</font><br/>Geri d&ouml;nmek i&ccedil;in <a href=dataChatActions.php>tıklatınız</a>";
	  ...
	  die("<p>&nbsp;</p><font id='hata'>Bu sayfa i&ccedil;in &ouml;ğrenci veya &ouml;ğretmenlerin işlem yapma izni yoktur!</font>");
