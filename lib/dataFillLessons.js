...
/*
konuSec2:
alt seçeneklerin oturumdan istenmesi
*/
...
function konuHazirla(konuNo){    
    ...
    if (httpObject2 != null && konuNo>0) {
		...
		httpObject2.setRequestHeader('Content-Type','application/x-www-form-urlencoded; charset=iso_8859-9');
...
function yorumGonder(konuID, comment){    
    ...
    if (httpObject4 != null) {
		...
		httpObject4.setRequestHeader('Content-Type','application/x-www-form-urlencoded; charset=iso_8859-9');
...
function cevapDegerlendir(cevap, id){    
    ...
    if (httpObjectCevap != null) {
		...
		httpObjectCevap.setRequestHeader('Content-Type','application/x-www-form-urlencoded; charset=iso_8859-9');
	 ...
	 if(httpObject.status == 200 || httpObject.status == 304){
		if(trim(httpObject.responseText)!="")
		  if(trim(httpObject.responseText)!="1") {			
			...
			w.document.open();
			w.document.writeln("<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\" \"http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd\"><html xmlns=\"http://www.w3.org/1999/xhtml\"><head><meta http-equiv=\"Content-Type\" content=\"text/html; charset=iso_8859-9\" />");
