...
function konuHazirla(konuNo){    
    httpObject = getHTTPObject();
    if (httpObject != null && konuNo>0) {
        httpObject.open("POST", "getContent.php", true);
		httpObject.setRequestHeader('Content-Type','application/x-www-form-urlencoded; charset=iso-8859-9');
  		httpObject.send('konu='+encodeURIComponent(konuNo));
        httpObject.onreadystatechange = function(){
			if(httpObject.readyState == 4 && (httpObject.status == 200 || httpObject.status == 304)){					
