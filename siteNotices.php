...
if ($tur=="2")	{//yönetici ise
  ...
  if($_GET["writable"]=="1") {
	 ...
	 echo "<font id='tamam'> Artık yazılabilir.</font>";
  }
	
  if($_GET["readonly"]=="1") {
	 @chmod($_uploadFolder,0755);		  
	 echo "<font id='tamam'> Artık salt okunur.</font>";
  }
	...
	if(empty($dosyUpload))
		  echo "<font id='uyari'> Veritabanı ile paylaşım klasörü ($_uploadFolder) uyumludur!</font>";
	  else {
		  echo "<font id='hata'> Veritabanı ile paylaşım klasörü ($_uploadFolder) uyumlu değildir!<br/>$dosyUpload<br/>";
...
<hr noshade="noshade" />
                  ...
                  * Siteyi bakıma almak için <a href="siteNotices.php?lock=1">tıklatınız</a>. <br />
                    Sitenin tekrar aktif olabilmesi için 
                    "siteLock.php" dosyasının adını "_siteLock.php" olarak değiştiriniz.</p>
                  <?php
	}
	
	if(substr(sprintf('%o', fileperms($_uploadFolder)), -4)=="0755"){
		echo "<p>** <strong>$_uploadFolder</strong> dizini sadece okunabilirdir. Yazılabilir yapmak için <a href=\"siteNotices.php?writable=1\">tıklatınız</a>.</p>";
	}else{
		echo "<p>** <strong>$_uploadFolder</strong> dizini yazılabilirdir. Salt okunur yapmak için <a href=\"siteNotices.php?readonly=1\">tıklatınız</a>.</p>";
	}
